...
class AttendanceRepository {
    async createRecord(data) {
        const { id, company_id, collaborator_id, schedule_id, type, lat, lng, marking_zone_id, is_valid_zone, status } = data;
    ...
    async findTodayRecords(companyId, collaboratorId) {
    ...
    async findTodaySchedule(companyId, collaboratorId) {
        const [rows] = await database_1.default.execute(`
    ...
    async findCollaboratorByIdentification(companyId, identification) {
        const [rows] = await database_1.default.execute(`
