...
export class BiometricRepository {
  ...
  // --- HUELLAS DACTILARES ---
  async saveFingerprint(data: { id: string, company_id: string, collaborator_id: string, finger_name: string, template: string, device_info?: string }) {
    await pool.execute(
      ...
      [data.id, data.company_id, data.collaborator_id, data.finger_name, data.template, data.device_info]
    );
  }
